...
* Input: the number of test cases $T$ followed by $T$ test cases.
  ...
  * $S$ consists of ASCII alphanumeric characters and spaces.
  * $1 ≤ N ≤ |S|$; $|S| ≥ 2$.
* Output: The answer for each test case on its own line.
-/

def main := mainWrapper (Σ t : Nat, Vector (Nat × ByteArrayNL) t) fun ⟨_, testcases⟩ => do
  for ⟨index, string⟩ in testcases do
    let string := (string : ByteArray)
